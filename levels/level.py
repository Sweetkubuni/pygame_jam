...
class Level:
    def __init__(self, state, tilemap, background, start_x, start_y) -> None:
        ...
        self.camera = Camera(self.state.game.player, self.state.game.GAME_WIDTH, self.state.game.GAME_HEIGHT, self.level_surface, self.state.game)
        ...
        self.music_loop = self.state.all_sounds["jinna shovel mario theme"] 
        self.music_loop.play(-1)
    def update(self):
        ...
        if self.state.game.player.dead:
            self.music_loop.stop()
            if self.state.game.player.dead_timer < 0:
                game_over = Game_over(self.state.game, self.state.game.player.coins, self.state.game.player.kills, self.state.game.player.dead_depth)
        ...
        for enemy in self.areas[self.current_area][1]:
            ...
            if enemy.off_bounce: enemy_remove_list.append(enemy)
            if enemy.dead:
                enemy_remove_list.append(enemy)
                self.state.game.player.kills += 1
                pygame.mixer.find_channel(True).play(self.state.game.player.sounds["puff"])
                self.particles.append(Particle(self.tilemap, pygame.Rect(enemy.rect.centerx-8, enemy.rect.centery-8, 16,16), self.state.all_animations["puff"], False, (8,8), 1, 40, [0,0], 0, False))
        ...
        for tile in self.areas[self.current_area][0].sprites():
            if tile.destructable: # The tile is a block
                ...
                if tile.delete:
                    self.state.game.player.attack_timer = -30
                    pygame.mixer.find_channel(True).play(tile.sounds["delete"])
                    block_remove_list.append(tile)
                    if tile.tile_id == 0 or tile.tile_id == 1:
                        self.particles.append(Particle(self.tilemap, pygame.Rect(tile.rect.centerx-5, tile.rect.centery-5, 10,10), self.state.all_animations["break particle"], False, (0,0), 2, 34, [1.047,2.094], 1.8, True))
                    if tile.tile_id == 4:
                        self.particles.append(Coin(pygame.Rect(tile.rect.centerx-6, tile.rect.y-6, 13, 15), self.state.all_animations["coin"], 450, 1.57, 1.5, self.state.all_sounds, self.state.game))
                        self.particles.append(Coin(pygame.Rect(tile.rect.centerx-6, tile.rect.y-6, 13, 15), self.state.all_animations["coin"], 450, 1.37, 1.3, self.state.all_sounds, self.state.game))
                        self.particles.append(Coin(pygame.Rect(tile.rect.centerx-6, tile.rect.y-6, 13, 15), self.state.all_animations["coin"], 450, 1.77, 1, self.state.all_sounds, self.state.game))
